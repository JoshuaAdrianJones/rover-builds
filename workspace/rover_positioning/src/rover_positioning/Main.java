...
public class Main {
	public static void main(String[] args) {
		...
		String pattern = "\\d{1,9}\\s\\d{1,9}|\\d{1,9}\\d{1,9}\\s\\d{1,9}\\d{1,9}|\\d{1,9}\\d{1,9}\\d{1,9}\\s\\d{1,9}\\d{1,9}\\d{1,9}";
		//String[] split = size.split(" ");
		while(size.matches(pattern)==false || size.contains("0")==true) {//error message
		...
		pattern = "\\d|\\d\\d|\\d\\d\\d";
		while(Integer.toString(numberOfRovers).matches(pattern)==false && numberOfRovers<=0) {//error message
	
			System.out.println("ERROR Please enter a valid input");
			System.out.println("How many rovers are there on the plateau? (for our test case this should be 2)");
			numberOfRovers = sc.nextInt();//lets user input number of rovers
		    }
