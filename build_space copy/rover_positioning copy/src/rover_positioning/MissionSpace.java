...
//defines mission space that rover's operate in
	...
	public void setRovers(Rover[] rovers){
		...
		for (int i=0; i<rovers.length; i++){
			...
			String input = roverInfo.nextLine();
			String pattern = "\\d\\s\\d\\s\\w|\\d\\d\\s\\d\\d\\s\\w|\\d\\d\\d\\s\\d\\d\\d\\s\\w";
			while(input.matches(pattern)==false) {//error message
		
				System.out.println("ERROR Please enter a valid input");
				System.out.println("please enter rover "+(i+1)+ " co-ordinates");
				input=roverInfo.nextLine();
			    }
			rovers[i].setStartCo(input);
			
			System.out.println("please enter rover "+(i+1)+ " Move Instructions");
			input = roverInfo.nextLine();
			pattern = "[MLR]+";
			while(input.matches(pattern)==false) {//error message
			...
			rovers[i].setMissionOrder(input);
		}
		roverInfo.close();
	}
